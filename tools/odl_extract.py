...
MIN_ORPHAN_CODE_LINES = 2      # tek Courier satırı listing değil (caption dosya adı)
...
def _is_code_region(item):
    return "code" in item


def _encloses(element, region):
    return _bbox(element)[1] <= region["bottom"] and _bbox(element)[3] >= region["top"]


def _is_orphan_listing(region, elements):
    """ODL'nin hiç üretmediği bir Courier bölgesi gerçekten listing mi? Bir ODL
    öğesinin içine düşen Courier parçası satır içi koddur, tek satırlık bölge ise
    listing caption'ındaki Courier-Bold dosya adıdır; ikisi de kod bloğu olmaz."""
    return (len(region["code"].splitlines()) >= MIN_ORPHAN_CODE_LINES
            and not any(_encloses(element, region) for element in elements))


def _settled_orphans(elements, regions, covered):
    """Listing olmayan yetim bölgeler; akışa girmemeleri için baştan yerleşmiş
    sayılır."""
    return {index for index, region in enumerate(regions)
            if index not in covered and not _is_orphan_listing(region, elements)}


def _regions_above(regions, settled, limit):
    """Sırası gelmiş ve henüz yerleşmemiş kod bölgeleri; yerleşti işaretlenir."""
    above = [(index, region) for index, region in enumerate(regions)
             if index not in settled and region["top"] > limit]
    settled.update(index for index, _ in above)
    return [region for _, region in sorted(above, key=lambda pair: -pair[1]["top"])]


def _reading_order(elements, regions):
    """Öğeleri ve kod bölgelerini okuma sırasında tek akışta birleştirir. Kod
    kutusunun içine düşen ODL öğeleri bölgeye devredilir; ODL gölgeli kutu için
    hiç öğe üretmediğinde (sayfa 130) bölge kendi dikey konumuna yerleşir."""
    matches = [_region_index(element, regions) for element in elements]
    settled = _settled_orphans(elements, regions, set(matches) - {None})
    items = []
    for element, index in zip(elements, matches):
        items.extend(_regions_above(regions, settled, _bbox(element)[3]))
        if index is None:
            items.append(element)
        elif index not in settled:
            settled.add(index)
            items.append(regions[index])
    items.extend(_regions_above(regions, settled, float("-inf")))
    return items


def _build_blocks(elements, layout, fixer):
    blocks = []
    for item in _reading_order(elements, _code_regions(layout)):
        if _is_code_region(item):
            blocks.extend(_caption_blocks(item, blocks))
            blocks.append(_code_block(item))
        else:
            blocks.extend(_element_blocks(item, fixer))
