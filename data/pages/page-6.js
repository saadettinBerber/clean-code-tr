  ...
  "concepts": [
    {
      ...
      "summary": {
        "en": "Martin compares programmers' responsibility to defend code quality with a doctor defending hand-washing. Even when the patient (manager/client) complains about \"wasting time,\" the doctor doesn't compromise on hygiene — because they know the risks. Similarly, programmers shouldn't compromise code quality despite \"deliver fast\" pressure.",
        "tr": "Martin, programcıların kodu savunma sorumluluğunu bir doktorun el yıkamayı savunmasına benzetir. Hasta (yönetici/müşteri) \"zaman kaybı\" diye şikâyet etse bile, doktor hijyen kurallarından ödün vermez — çünkü riskleri bilir. Aynı şekilde programcılar da \"hızlı teslim et\" baskısına rağmen kod kalitesinden ödün vermemelidir."
      },
      "bad": {
        "lang": "javascript",
        "code": "// Yönetici: \"Bu özellik yarına hazır olmalı!\"\n// Geliştirici: \"Tamam, hemen yaparım...\"\n\nfunction calculateDiscount(cart) {\n    // TODO: Validation eklenecek (sonra...)\n    // TODO: Edge case'ler handle edilecek (sonra...)\n    // TODO: Test yazılacak (sonra...)\n    let d = 0;\n    for (let i = 0; i < cart.length; i++) {\n        if (cart[i].q > 5) d += cart[i].p * 0.1;\n        if (cart[i].q > 10) d += cart[i].p * 0.05;\n        if (cart[i].vip) d += cart[i].p * 0.15;\n    }\n    return d;\n}\n// 6 ay sonra: 47 bug raporu, kimse koda dokunamıyor",
        "why": {
          "en": "Bending to pressure. The developer bent to pressure and skipped validation, tests, and meaningful naming. Every \"we'll do it later\" turned into technical debt.",
          "tr": "Baskıya boyun eğmek. Geliştirici baskıya boyun eğip validasyon, test ve anlamlı isimlendirme gibi temel pratikleri atlamış. \"Sonra yaparız\" dediği her şey teknik borca (technical debt) dönüşmüş."
        }
      },
      "good": {
        "lang": "typescript",
        ...
        "why": {
          "en": "Professional stance",
          "tr": "Profesyonel duruş"
        }
      },
      "tip": {
        "en": "It's not about saying \"No\" but \"Yes, but...\" to your manager. \"I can do this in 1 day but with 3 critical risks. In 2 days I can do it risk-free. Which do you prefer?\" This shows both professionalism and solution-oriented thinking.",
        "tr": "Yöneticinize \"Hayır\" demek değil, \"Evet, ama...\" demek önemlidir. \"Bunu 1 günde yapabilirim ama 3 kritik riski var. 2 günde risksiz yapabilirim. Hangisini tercih edersiniz?\" Bu yaklaşım hem profesyonelliğinizi hem de çözüm odaklılığınızı gösterir."
      }
    },
    {
      "id": "conundrum",
      ...
      "summary": {
        "en": "The fundamental conundrum: we <em>know</em> messes slow us down, yet we feel pressure to make messes to meet deadlines. Martin states the resolution clearly: the only way to go fast is to keep the code clean. Making a mess doesn't speed you up — it slows you down instantly.",
        "tr": "Programcıların temel ikilemi şudur: Karmaşanın (mess) bizi yavaşlattığını <em>biliyoruz</em>, ama son teslim tarihlerine (deadlines) yetişmek için karmaşa yaratma baskısı hissediyoruz. Martin bu ikilemin çözümünü açıkça belirtir: Hızlı gitmenin tek yolu kodu temiz tutmaktır. Karmaşa yapmak sizi hızlandırmaz — anında yavaşlatır.",
        "html": true
      },
      "bad": {
        "lang": "python",
        "code": "# Sprint 1: \"Hızlıca halledelim\"\ndef send_notification(user_id, msg, type, urgent, channel):\n    if type == \"email\":\n        if urgent:\n            # acil email gönder\n            smtp = smtplib.SMTP('smtp.company.com')\n            smtp.sendmail('no-reply@co.com', get_email(user_id), msg)\n        else:\n            # normal email\n            smtp = smtplib.SMTP('smtp.company.com')\n            smtp.sendmail('no-reply@co.com', get_email(user_id), msg)\n    elif type == \"sms\":\n        if urgent:\n            # acil sms\n            requests.post('https://sms-api.com/send',\n                json={'to': get_phone(user_id), 'msg': msg, 'priority': 'high'})\n        else:\n            requests.post('https://sms-api.com/send',\n                json={'to': get_phone(user_id), 'msg': msg})\n    elif type == \"push\":\n        # ... 50 satır daha\n        pass\n\n# Sprint 3: \"Slack bildirimi de ekleyelim\" → 200 satırlık if/elif\n# Sprint 5: \"Neden bu kadar yavaş ilerliyoruz?!\"",
        "why": {
          "en": "\"Let's just get it done\" mentality. The function growing with \"let's add it quickly\" each sprint becomes an unmaintainable monster. Shortcuts taken to gain speed quickly turn into chains that slow you down.",
          "tr": "\"Hızlıca halledelim\" zihniyeti. Her sprint'te \"hızlıca ekleyelim\" diyerek büyüyen fonksiyon, birkaç sprint sonra değiştirilmesi imkânsız bir canavara dönüşür. Hız kazanmak için yapılan kestirmeler, çok geçmeden sizi yavaşlatan zincirlere dönüşür."
        }
      },
      "good": {
        "lang": "python",
        "code": "# Sprint 1: Biraz daha zaman alır ama sürdürülebilir\nclass NotificationService:\n    def __init__(self):\n        self._channels = {}\n\n    def register(self, channel_type, handler):\n        self._channels[channel_type] = handler\n\n    def send(self, user_id, message, channel_type, urgent=False):\n        handler = self._channels.get(channel_type)\n        if not handler:\n            raise UnsupportedChannel(channel_type)\n        handler.send(user_id, message, urgent)\n\nclass EmailChannel:\n    def send(self, user_id, message, urgent):\n        priority = Priority.HIGH if urgent else Priority.NORMAL\n        self.mailer.send(\n            to=self.user_repo.get_email(user_id),\n            body=message,\n            priority=priority\n        )\n\n# Sprint 3: \"Slack ekleyelim\" → sadece SlackChannel sınıfı yaz, register et. Bitti.\n# Sprint 5: \"Harika ilerliyoruz!\"",
        "why": {
          "en": "Clean structure = sustainable speed",
          "tr": "Temiz yapı = sürdürülebilir hız"
        }
      },
      "tip": {
        "en": "Before starting a task with \"let's just get it done,\" ask yourself: \"If I need to change this code in 3 months, how many hours will today's 'quick' solution cost me then?\" Usually spending 30 extra minutes today saves 3 days in the future.",
        "tr": "Bir görevi \"hızlıca halledelim\" diye başlamadan önce kendinize sorun: \"Bu kodu 3 ay sonra değiştirmem gerekirse, bugünkü 'hızlı' çözüm o zaman bana kaç saat kaybettirecek?\" Genellikle bugün 30 dakika fazla harcamak, gelecekte 3 gün kazandırır."
      }
    },
    {
      "id": "art",
      ...
      "summary": {
        "en": "Martin compares writing clean code to painting: being able to tell good art from bad doesn't mean you can paint. Similarly, recognizing clean code from dirty code doesn't mean you can <em>write</em> clean code. Writing clean code is a craftsmanship skill — it requires time, practice, and discipline to learn.",
        "tr": "Martin, temiz kod yazmayı resim yapmaya benzetir: İyi bir resmi kötüsünden ayırt edebilmek, resim yapmayı bildiğiniz anlamına gelmez. Aynı şekilde temiz kodu kirli koddan ayırt edebilmek, temiz kod <em>yazabildiğiniz</em> anlamına gelmez. Temiz kod yazmak bir zanaat (craftsmanship) becerisidir — öğrenilmesi zaman, pratik ve disiplin gerektirir.",
        "html": true
      },
      "bad": {
        "lang": "java",
        "code": "// Geliştirici biliyor ki bu kod kötü... ama alternatifi ne?\npublic class DataProcessor {\n    public Object process(Object data, int mode, boolean flag1,\n                          boolean flag2, String format, int retry) {\n        Object result = null;\n        if (mode == 1) {\n            if (flag1) {\n                if (format.equals(\"json\")) {\n                    // 30 satır iç içe mantık\n                    result = doSomething(data);\n                } else if (format.equals(\"xml\")) {\n                    result = doSomethingElse(data);\n                }\n            } else if (flag2) {\n                // 20 satır daha...\n                result = doAnotherThing(data);\n            }\n        } else if (mode == 2) {\n            // mode 1'in kopyası, küçük farkla...\n        }\n        return result;\n    }\n}",
        "why": {
          "en": "\"I know it's bad but I don't know how to fix it\". The developer can feel this code is bad but without developed \"code-sense,\" they don't know how to fix it. Recognizing problems is the first step, but it's not enough.",
          "tr": "\"Kötü olduğunu biliyorum ama nasıl düzelteceğimi bilmiyorum\". Geliştirici bu kodun kötü olduğunu hissedebilir ama \"kod duygusu\" (code-sense) gelişmediği için nasıl düzelteceğini bilemez. Sorunları tanımak ilk adımdır, ama yeterli değildir."
        }
      },
      "good": {
        "lang": "java",
        "code": "// Strateji deseni (Strategy Pattern) + açık arayüz\npublic interface DataFormatter {\n    FormattedData format(RawData data);\n}\n\npublic class JsonFormatter implements DataFormatter {\n    public FormattedData format(RawData data) {\n        return new FormattedData(toJson(data));\n    }\n}\n\npublic class DataPipeline {\n    private final DataFormatter formatter;\n    private final RetryPolicy retryPolicy;\n\n    public DataPipeline(DataFormatter formatter, RetryPolicy retryPolicy) {\n        this.formatter = formatter;\n        this.retryPolicy = retryPolicy;\n    }\n\n    public ProcessedData process(RawData data) {\n        RawData validated = validate(data);\n        FormattedData formatted = formatter.format(validated);\n        return retryPolicy.execute(() -> save(formatted));\n    }\n}",
        "why": {
          "en": "Restructured with code-sense",
          "tr": "Kod duygusu ile yeniden yapılandırılmış"
        }
      },
      "tip": {
        "en": "To develop \"code-sense\": spend 15 minutes daily reading open source code. Study core modules of well-known projects (Spring, React, Django). Ask \"Why is this written this way?\" Over time, you'll start recognizing and applying patterns.",
        "tr": "\"Kod duygusu\" geliştirmek için: Her gün 15 dakika açık kaynak kodlarını okuyun. Özellikle iyi bilinen projelerin (Spring, React, Django) çekirdek modüllerini inceleyin. \"Bu neden böyle yazılmış?\" diye sorun. Zamanla kalıpları tanımaya ve uygulamaya başlarsınız."
      }
    }
  ]
});
